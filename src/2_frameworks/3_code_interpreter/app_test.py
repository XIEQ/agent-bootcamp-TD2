...
REACT_INSTRUCTIONS = """\
Answer the question using the Exploratory_Data_Analysis tool. \
EACH TIME before invoking the tool, you must explain your reasons for doing so. \
If the Exploratory_Data_Analysis tool did not return intended results, try again. \
For best performance, divide complex queries into simpler sub-queries. \
Do not make up information. \
If user ask to visualize data, format the plot.
"""
...
async def _main(question: str, gr_messages: list[ChatMessage]):
    ...
    main_agent = agents.Agent(
        ...
        tools=[
            eda_agent.as_tool(
                tool_name = "Exploratory_Data_Analysis",
                tool_description="Perform exploratory data analysis",
            )
        ],
        model=agents.OpenAIChatCompletionsModel(
...
demo = gr.ChatInterface(
    ...
    #],
    examples=[
        "What is the unique number of client_id in the final_unified_dataset_sample.csv?",
        "What is the unique number of card_id in the final_unified_dataset_sample.csv?",
        "Which client_id spent the most amount in the final_unified_dataset_sample?",
    ],
)
